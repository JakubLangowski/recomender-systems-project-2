...
class DataPreprocessingToolkit(object):
    ...
    @staticmethod
    def filter_out_company_clients(df: pd.DataFrame) -> pd.DataFrame:
        """
    ...
    @staticmethod
    def filter_out_long_stays(df: pd.DataFrame) -> pd.DataFrame:
        """
    ...
    @staticmethod
    def filter_out_low_prices(df: pd.DataFrame) -> pd.DataFrame:
        """
    ...
    @staticmethod
    def fix_date_to(df: pd.DataFrame) -> pd.DataFrame:
        """
    ...
    @staticmethod
    def add_book_to_arrival(df: pd.DataFrame) -> pd.DataFrame:
        """
        ...
        :rtype: pd.DataFrame
        """
        df['book_to_arrival'] = df['date_from'] - df['booking_date']
        df['book_to_arrival'] = df['book_to_arrival'].dt.days
        # df['book_to_arrival'] = df.apply(lambda x: (x['date_from'].date() - x['booking_date'].date()).days, axis=1)
        return df

    @staticmethod
    def add_nrooms(df: pd.DataFrame) -> pd.DataFrame:
        """
    ...
    @staticmethod
    def add_weekend_stay(df: pd.DataFrame) -> pd.DataFrame:
        """
        ...
        :rtype: pd.DataFrame
        """
        df['weekend_stay'] = df.apply(
            lambda x: np.busday_count(x['date_from'].date(), x['date_to'].date(), weekmask=[0, 0, 0, 0, 1, 1, 0]) > 0,
            axis=1)
        df['weekend_stay'] = df['weekend_stay'].map({True: 'True', False: 'False'})
        return df

    @staticmethod
    def add_night_price(df: pd.DataFrame) -> pd.DataFrame:
        """
    ...
    @staticmethod
    def clip_book_to_arrival(df: pd.DataFrame) -> pd.DataFrame:
        """
    ...
    @staticmethod
    def sum_npeople(df: pd.DataFrame) -> pd.DataFrame:
        """
        ...
        :rtype: pd.DataFrame
        """
        df['n_people'] = df[['n_people', 'n_children_1', 'n_children_2', 'n_children_3']].sum(axis=1)
        return df

    @staticmethod
    def leave_one_from_group_reservations(df: pd.DataFrame) -> pd.DataFrame:
        """
    ...
    def aggregate_group_reservations(self, df):
        ...
        group_reservations: pd.DataFrame = df.loc[df['group_id'] != ""]
        ...
        for col in self.mode_columns:
            options[col] = lambda x: x.value_counts().idxmax()
        for col in self.first_columns:
            options[col] = lambda x: x.iloc[0]

        group_reservations = grouped.agg(options)
    ...
    @staticmethod
    def leave_only_ota(df: pd.DataFrame) -> pd.DataFrame:
    ...
    @staticmethod
    def add_interaction_id(df: pd.DataFrame) -> pd.DataFrame:
